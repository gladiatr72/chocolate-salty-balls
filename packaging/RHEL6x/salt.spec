...
%post -n salt-master
  bits="salt-master salt-minion"

  for b in $bits;
  do
          /sbin/chkconfig --add $b
          status=$( /sbin/chkconfig --list $b \
            | /bin/sed -nre '/0:/s/^.*3:(on|off).*4.*/\1/p' )

          if [[ $status == "on" ]];
          then
            [[ -f /var/run/$b.pid ]] &&
              /bin/ps  -p $( cat /var/run/$b.pid ) >& /dev/null ;
              [[ $? -eq 0 ]] &&
              /bin/kill -9 $( cat /var/run/$b.pid )

            /sbin/service $b start
          fi
  done
...
%changelog
* Thu Feb 20 2014 Stephen Spencer <gladiatr72@gmail.com> - 2014.1.1-5
- Update to bugfix release 2014.1.1
- Fix non-systemctl pre/post scripts to actually work when upgrading a minion 
